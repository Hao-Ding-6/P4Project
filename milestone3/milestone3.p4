...
const bit<48> time_threshold = 1500;
...
control myIngress(inout headers hdr,
    ...
    // store the previous time and port
    register<bit<48>>(1000) time_reg;
    register<bit<9>>(1000) port_reg;
    ...
    apply {
        if (hdr.ipv4.isValid() && hdr.ipv4.ttl > 0) {
            ...
            if (hdr.ecmp.is_load_balance == 1) {
                ...
                // read previous time and port
                bit<48> pre_time;
                time_reg.read(pre_time, hash_value);
                bit<9> pre_port;
                port_reg.read(pre_port, hash_value);

                // first time
                if (pre_time == 0) { 
                    standard_metadata.egress_spec = 2;
                    hdr.ethernet.srcAddr = hdr.ethernet.dstAddr;
                    hdr.ethernet.dstAddr = 0x000210;
                    hdr.ipv4.ttl = hdr.ipv4.ttl - 1;
                    time_reg.write(hash_value, cur_time);
                    port_reg.write(hash_value, 2);
                    
                // exceed time threshold
                } else if (cur_time - pre_time > time_threshold) {
                    if (pre_port == 2) {
                        standard_metadata.egress_spec = 3;
                        hdr.ethernet.dstAddr = 0x000310;
                        port_reg.write(hash_value, 3);
                    } else if (pre_port == 3) {
                        standard_metadata.egress_spec = 2;
                        hdr.ethernet.dstAddr = 0x000210;
                        port_reg.write(hash_value, 2);
                    ...
                    hdr.ethernet.srcAddr = hdr.ethernet.dstAddr;
                    hdr.ipv4.ttl = hdr.ipv4.ttl - 1;
                // normal case, follow the last packet
                } else {
                    standard_metadata.egress_spec = pre_port;
                    if (pre_port == 2) {
                        hdr.ethernet.dstAddr = 0x000210;
                    } else if (pre_port == 3) {
                        hdr.ethernet.dstAddr = 0x000310;
